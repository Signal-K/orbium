        ...
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="bg-aqua-400 p-2 rounded-md border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                <Microscope className="h-8 w-8 text-white" />
              </div>
              <span className="text-2xl font-bold">Star Sailors</span>
            </div>
            <nav className="hidden md:flex space-x-6">
              {/* {["Repositories", "Data Sources", "About", "Docs"].map((item, index) => ( */}
              {["Open-Source"].map((item, index) => (
                <Link
                  key={item}
                  href="https://github.com/signal-k"
                  className="text-sm font-medium bg-white px-4 py-2 border-2 border-black rounded shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] hover:translate-x-[2px] hover:shadow-none transition-all"
                >
                  {item}
                </Link>
              ))}
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-wrap gap-6">
                <Link href="https://starsailors.space/auth">
                    <Button
                        size="lg"
                        className="bg-aqua-400 text-white hover:bg-aqua-400 hover:translate-y-[-2px] hover:translate-x-[2px] text-lg px-8 py-6 rounded-md border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all hover:shadow-none"
                    >   
                        Login
                    </Button>
                </Link>
      ...
      <section className="py-20 px-6 bg-gray-50">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-8">
          {partners.map((partner, index) => (
            <Card key={index} className="max-w-xs p-6 flex flex-col items-center text-center border border-gray-300 shadow-md">
              <Image src={partner.logo} alt={partner.name} width={100} height={100} className="rounded-lg" />
              <h3 className="text-lg font-semibold mt-4">{partner.name}</h3>
              <p className="text-sm text-gray-700 mt-2">{partner.description}</p>
              <Link href={partner.url} className="mt-4 text-aqua-500 hover:underline">
                Learn More
              </Link>
            </Card>
          ))}
